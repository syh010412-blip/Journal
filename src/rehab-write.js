...
async function appendBlocksInChunks(pageId, blocks) {
  ...
  for (let i = 0; i < blocks.length; i += CHUNK) {
    ...
    console.log(`[rehab-write] 블록: ${Math.min(i + CHUNK, blocks.length)}/${blocks.length}`);
  }
}

async function upsertRehabReportPage(stats, blocks) {
  ...
  const title = `주간 재활 리포트 ${startDate?.replace(/-/g, '.')}`;

  console.log(`[rehab-write] 페이지: "${title}"`);
  const existing = await findExistingPage(REPORT_DB_ID, title);

  if (existing) {
    console.log('[rehab-write] 기존 페이지 업데이트…');
    await clearPageBlocks(existing.id);
    await appendBlocksInChunks(existing.id, blocks);
    console.log('[rehab-write] 업데이트 완료');
    return existing.id;
  }
  ...
  console.log('[rehab-write] 새 페이지 생성 완료');
  return page.id;
}

module.exports = { upsertRehabReportPage };
