...
function buildCalendarSummary(calStats) {
if (!calStats || calStats.total === 0) return '(Google Calendar 데이터 없음)';

const DAY_NAMES = ['일', '월', '화', '수', '목', '금', '토'];
const dayLine = calStats.dayStats
  .map((d, i) => `${DAY_NAMES[i]}요일: ${d.count}건`)
  .filter(s => !s.endsWith('0건'))
  .join(', ');

const topEventsLine = calStats.topTitles.slice(0, 10)
  .map(t => `${t.title}(${t.count}회)`).join(', ');
...
const datelines = Object.entries(byDate)
  ...
  .map(([date, evs]) => {
    ...
    return `  ${date}(${day}) [${evs.map(e => e.title).join(', ')}]`;
  }).join('\n');

return `- 총 이벤트: ${calStats.total}건\n- 요일별: ${dayLine}\n- 자주 등장한 활동: ${topEventsLine}\n\n## 날짜별 캘린더 상세\n${datelines}`;
}

function buildPrompt(stats, calStats) {
...
const calSection = `\n## Google Calendar 데이터\n\n${buildCalendarSummary(calStats)}\n`;

const focusSection = ANALYSIS_FOCUS.length > 0
? `\n## 분석 포인트\n${ANALYSIS_FOCUS.map(f => `- ${f}`).join('\n')}\n`
: '';

return `아래는 개인 일기 + Google Calendar 데이터입니다. 두 데이터를 종합하여 JSON을 출력하세요.
${focusSection}${calSection}
## 일기 전체 현황
...
{
"overallSummary": "일기+캘린더를 종합한 한 주 총평 3~4문장",
"recordingHabit": {
...
"calendarInsight": {
"summary": "캘린더에서 보이는 한 주 생활 패턴 2~3문장",
"activityBalance": "치료/일/자유시간/자기관리 등 활동 균형 평가",
"suggestion": "캘린더 기반 생활 개선 제안 1~2문장"
},
"weekdayPattern": {
...
"growthPoints": {
"strengths": "일기+캘린더에서 드러나는 심리적 강점이나 긍정적 변화",
...
async function analyzeDiaryData(stats, calStats) {
...
max_tokens: 4000,
system: `당신은 따뜻하고 통찰력 있는 일기 분석 코치입니다. 개인 일기와 Google Calendar 데이터를 종합하여 감정 패턴, 생활 루틴, 심리적 경향, 성장 포인트를 발견해 줍니다. 반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이. 한국어로. ${emphasis}. ${tone} 체 사용.`,
messages: [{ role: 'user', content: buildPrompt(stats, calStats) }],
});
